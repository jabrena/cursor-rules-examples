Feature: God Mythology API Concurrent Access and Thread Safety
  ...
  Scenario: Mixed concurrent requests ensure overall system thread safety
    When many users concurrently request information for GREEK, ROMAN, NORDIC, INDIAN, CELTIBERIAN gods simultaneously
    ...
    And all requests for "INDIAN" gods retrieve the correct list of Indian gods
    And all requests for "CELTIBERIAN" gods retrieve the correct list of Celtiberian gods
